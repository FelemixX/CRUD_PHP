...
1. Добавить поле ИНН для клиента (сделал в документе, чтобы была логика) [DONE]
2. Проверять валидность ИНН в базе ФНС. Если ИНН не валидный, то создание документа прерывается. [DONE]
